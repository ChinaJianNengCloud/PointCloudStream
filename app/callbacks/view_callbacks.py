...
def on_acq_mode_toggle_state_changed(self: "SceneStreamer"):
    logger.debug("Acquisition mode state changed")

def on_display_mode_combobox_changed(self: "SceneStreamer", text):
    self.display_mode = text
    logger.debug(f"Display mode changed to: {text}")

def on_center_to_robot_base_toggle_state_changed(self: "SceneStreamer"):
